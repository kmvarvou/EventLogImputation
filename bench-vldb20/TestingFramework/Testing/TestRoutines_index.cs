            ...
            switch (et)
            {
                ...
                case ExperimentType.Recovery:
                    switch (es)
                    {
                        ...
                        case ExperimentScenario.MultiColumnDisjoint:
                                string inputFile = $"{DataWorks.FolderData}{code}/{code}_normal.txt";
                        ...
                        case ExperimentScenario.MissingSubMatrix:
                            string inputFile2 = $"{DataWorks.FolderData}{code2}/{code2}_normal.txt";
                            Console.WriteLine(System.IO.Directory.GetCurrentDirectory());
                            
                    List<(int, int)> missing = new List<(int, int)>();
                            
			     string filePath = $"/home/kostis/Downloads/bench-vldb20-master/TestingFramework/bin/Debug/Results/cdrec_0.2_missingness/{code2}/index/index.txt";
    				...
    				if (parts.Length >= 2 && int.TryParse(parts[0].Trim(), out int col) && int.TryParse(parts[1].Trim(), out int row))
    {
        missing.Add((col, row));
    }
    else
    {
        Console.WriteLine($"Could not parse integers from line: {line}");
    }
                        }
                    }
                    else
                    {
                        throw new FileNotFoundException("Specified index file not found");
                    }

                    // Convert the list of missing positions to blocks
